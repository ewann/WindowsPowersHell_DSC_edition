...
* Click ```Create Stack```
* For the option ```Choose a template``` click the ```Browse``` button and choose the ```WindowsPowersHell_DSC_edition_CF_stack_01.json```  from the {local-repo} location where we cloned the repo earlier
* Click ```Next```
...
*  On the Windows server open a new PowerShell window. Execute:
